...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // Parameters
        //
        // If Qt is used, parameters/bitcrystal.conf are parsed in qt/bitcoin.cpp's main()
        ...
        {
            // First part of help message is specific to bitcrystal / RPC client
            std::string strUsage = _("BitCrystal version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  bitcrystal [options]                     " + "\n" +
                  "  bitcrystal [options] <command> [params]  " + _("Send command to -server or bitcrystal") + "\n" +
                  "  bitcrystal [options] help                " + _("List commands") + "\n" +
                  "  bitcrystal [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "bitcrystal:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect bitcrystal signal handlers
    noui_connect();
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: bitcrystal.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: bitcrystal.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 9596 or testnet: 9597)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9108 or testnet: 9109)") + "\n" +
        ...
        "\n" + _("SSL options: (see the BitCrystal Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. BitCrystal is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("BitCrystal version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of BitCrystal") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart BitCrystal to complete") << "\n";
